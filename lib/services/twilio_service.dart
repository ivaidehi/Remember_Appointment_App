...
class TwilioService {
  ...
  Future<void> sendWhatsappMsg({
    ...
    required String date,
    required String timeSlot,
    required String whatsappMsg,
    BuildContext? context, // ✅ Make it optional
  }) async {
    try {
      if (toNumber.isNotEmpty && name.isNotEmpty) {
  ...
  void handleTwilioError(dynamic error, BuildContext? context) {
    if (context != null) {
      // ✅ Check if context is available before using ScaffoldMessenger
      ...
      print("Twilio Error: $error"); // ✅ Log error if no UI context
    }
  }
}
